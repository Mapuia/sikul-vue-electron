<template>
  <div class="form-container wide">
    ...
    <!-- Selection -->
    <div class="box">
      <h1 class="title has-text-centered is-4">Student Management (Personal Info)</h1>   
      
      <!-- Search -->
      <h2 class="title is-6 has-text-centered">Search Student</h2>
                ...
                <td>
                  <div class="buttons is-right">
                    ...
                    <button v-if="canAccess(['admin'])"
                        ...
                        @click="confirmDeleteStudent(student.Id, student.Name)"
                        title="Delete"
                      >
    ...
    <div class="modal" :class="{ 'is-active': showModal }">
      ...
      <div class="modal-card" style="width: 90%; max-width: 1000px;">
        <header class="modal-card-head">
          <p class="modal-card-title">Student Information (Personal)</p>
          <button class="delete" aria-label="close" @click="closeModal"></button>
        </header>
        <section class="modal-card-body">
          <PersonalInfoView v-if="selectedStudent && modalMode === 'view'" :student="selectedStudent" :admission="admission" />
...
import PersonalInfoEditForm from '@/components/PersonalInfoEditForm.vue';
...
async function viewStudentDetails(studentId) {
  ...
  const res = await window.electronAPI.getStudentPersonalInfo(studentId);
  if (res.success) {
...
async function confirmDeleteStudent(studentId, Name) {
  try {
    const confirmed = await window.electronAPI.showConfirmationDialog(
      `Are you sure you want to delete - ${Name} from the Database? This action cannot be undone.`,
    );
    if (!confirmed) return;
