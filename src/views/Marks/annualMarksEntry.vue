                ...
                <tr v-for="student in students" :key="student.StudentId">
                  ...
                  <td>
                    <input
                      ...
                      v-model.number="annualMarks[student.StudentId]"
                      @keydown.enter="handleEnterKey($event, student.StudentId)"
                      @input="updateStatus(student.StudentId)"
                    />
                  </td>
...
const calculateTotal = (studentId) => {
  ...
  const annual = annualMarks.value[studentId] || 0
...
async function loadExistingMarks() {
  try {
    ...
    result.forEach(mark => {
      ...
      annualMarks.value[mark.StudentId] = mark.TerminalMarksObtained || 0
...
async function saveMarks() {
  ...
  try {
    ...
    const marksData = students.value.map(student => {
      ...
      const terminalMarksObtained = annualMarks.value[student.StudentId] || 0;
