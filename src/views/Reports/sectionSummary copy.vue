  ...
  <div class="form-container full mt-5">
    <div v-if="resultPublished">
      <div class="has-text-centered mb-4">
        <h1 class="title is-4">Section Summary - {{ resultName }}</h1>
      </div>
            ...
            <div class="field">
              ...
              <div class="select is-fullwidth">
                <select v-model="selectedSectionId" :disabled="!selectedClassId || sections.length === 0">
                  ...
                  <option v-if="sections.length === 0" :value="0">All / No section</option>
                </select>
              </div>
            </div>
          </div>
    ...
    <div v-else class="is-flex is-justify-content-center is-align-items-center" style="height: 600px;">
      <div class="notification is-danger has-text-centered px-6">
...
watch(() => selectedClassId.value, async (newClass) => {
  ...
  await fetchSectionsForClass(newClass)
...
watch(route, async (newType) => {
  ...
  examType.value = newType.query.type || 'terminal'
  await getExamByType()
  await checkPublishStatus()
})

resultName.value = examType.value === 'terminal' ? 'Half Yearly Exam Result' : 
(examType.value === 'annual' ? 'Annual Exam Result' : examType.value === 'final' ? 'Final Exam Result' : 'Selection result')
